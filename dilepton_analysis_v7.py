...
class ttHbb(processor.ProcessorABC):
	...
	def process(self, events, parameters=parameters, samples_info=samples_info):
		...
		good_jets = jet_selection_v7(jets, muons, (good_muons|veto_muons), parameters["jets"]) & jet_selection_v7(jets, electrons, (good_electrons|veto_electrons), parameters["jets"])
		bjets_resolved = good_jets & (getattr(jets, parameters["btagging_algorithm"]) > parameters["btagging_WP"])
		good_fatjets = jet_selection_v7(fatjets, muons, good_muons, parameters["fatjets"]) & jet_selection_v7(fatjets, electrons, good_electrons, parameters["fatjets"])
		...
		for trigger in parameters["triggers"]["ee"]:	trigger_ee   = trigger_ee   & HLT[trigger.lstrip("HLT_")]
		for trigger in parameters["triggers"]["emu"]:	trigger_emu  = trigger_emu  & HLT[trigger.lstrip("HLT_")]
		for trigger in parameters["triggers"]["mumu"]:	trigger_mumu = trigger_mumu & HLT[trigger.lstrip("HLT_")]
		
		#if "DoubleMuon" in args.sample: trigger_ee = np.zeros(nEvents, dtype=np.bool)
		#if "DoubleElectron" in args.sample: trigger_emu = np.zeros(nEvents, dtype=np.bool)
		...
		mask_events_basic = ((selev.nleps == 2) & (selev.ngoodleps >= 1) & (ll.charge == 0) &
							(selev.MET.pt > parameters['met']) & (selev.nfatjets > 0) & (selev.btags >= parameters['btags']) &
